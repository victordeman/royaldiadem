    ...
    <main className="min-h-screen bg-background">
        ...
        <div className="grid gap-12 mt-8">
          {cases.map((cs, index) => (
            <motion.div
              ...
              className={`flex flex-col ${index % 2 === 1 ? 'lg:flex-row-reverse' : 'lg:flex-row'} gap-12 glass-premium rounded-[3rem] p-8 md:p-12 border border-slate-200 dark:border-slate-800 group`}
            >
              <div className="flex-1 relative overflow-hidden rounded-2xl aspect-video lg:aspect-square">
                 ...
                 <div className="absolute top-6 left-6 z-20">
                    <Badge variant="default" className="uppercase tracking-[0.2em]">{cs.industry}</Badge>
                 </div>
              </div>

              <div className="flex-1 flex flex-col justify-center space-y-8">
                 <div className="space-y-4">
                    <h2 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white group-hover:text-gold transition-colors">{cs.title}</h2>
                    <p className="text-lg text-slate-900 dark:text-slate-600 dark:text-slate-400 leading-relaxed">
                       {cs.description}
                    </p>
                 </div>

                 <div className="p-6 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-white/5 flex items-center justify-between">
                    <div>
                       <span className="text-xs uppercase tracking-widest text-slate-900 dark:text-white/40 block mb-1">Key Outcome</span>
                       <span className="text-xl font-bold text-gold">{cs.outcome}</span>
                    </div>
                    <Shield className="h-8 w-8 text-gold/20" />
                 </div>

                 <Button variant="ghost" className="text-slate-900 dark:text-white hover:text-gold p-0 flex items-center gap-2 group/btn w-fit h-auto">
      ...
      <Section className="bg-slate-50 dark:bg-slate-900/50">
        <div className="text-center space-y-12 max-w-3xl mx-auto">
           <h2 className="text-4xl font-bold text-slate-900 dark:text-white">Have a similar project?</h2>
           <p className="text-lg text-slate-900 dark:text-slate-600 dark:text-slate-400 leading-relaxed">
              Our technical team is ready to analyze your specific requirements and
              propose a high-precision manufacturing solution.
           </p>
           <Button variant="default" size="lg" className="px-0">
